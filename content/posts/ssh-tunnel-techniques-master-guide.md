...
**用途**：**ローカルPCから**閉域ネットワーク内のDB・管理画面**にアクセスする**（リモートサーバーのポートを手元に転送）
...
**用途**：**ローカルPC（NAT内）のサービスを**リモートサーバー経由で**外部に公開する**（手元のポートをリモートサーバーに転送）
...
**用途**：**ローカルPCの**ブラウザ・CLI・各種ツールから**リモートネットワークにまとめてアクセスする**（複数の接続先を1つのSOCKSプロキシで処理）
...
**用途**：**ローカルPCから**踏み台サーバー（Bastion）を経由して**内部サーバーにアクセスする**（直接接続できない環境を多段接続で突破）
...
**用途**：**サーバー上の定期ジョブから**SSHトンネル経由で**リモートリソース（DB等）にアクセスする**（自動化スクリプトでの接続）
...
**用途**：**ローカルPCから**SSH経由で**リモートサーバーのログを取得する**（踏み台経由での多段ログ取得にも対応）
...
**用途**：**ローカルPCから**AWS/GCPのプライベートリソース（EC2/RDS/GCE等）**にアクセスする**（IAM認証を活用し22番ポート開放不要）
...
**用途**：**ローカルPCの開発サーバー（Vite/BrowserSync）を**リモートサーバー経由で**実機（iPhone/Android）から確認する**（手元のポートをリモートに転送し実機プレビュー）
...
**用途**：**スマートフォン（iPhone/Android）から**ローカルPC経由のSSHトンネルで**内部API・閉域ネットワークにアクセスする**（Wi-Fiプロキシ設定でVPN不要）
...
**用途**：**ローカルPCから**指定したサブネット全体**に透過的にアクセスする**（ルーティングテーブルを書き換え、すべてのツールがプロキシ設定不要で動作）
...
**用途**：**ローカルPCのブラウザから**リモートサーバー上の**code-server（ブラウザ版VSCode）にアクセスする**（管理画面Grafana/Kibana等にも応用可能）
