...
重要なのは、**Consumer が失敗しても、Producer の処理自体は完了している**ということだ。

注文APIは「注文を受け付けました」と返していい。ただし、これは「注文が完了した」という意味ではなく、「注文処理を開始した」という意味だ。在庫引当が失敗した場合、それは在庫引当Consumer の責任範囲であり、注文APIとは独立して対処・復旧できる。

※ 注意：ビジネス的には在庫引当の失敗は注文全体に影響する。ここで言う「影響がない」とは、技術的な責任範囲の話であり、障害対応時に「どこを調査すべきか」「誰が対処すべきか」を明確にするための分離だ。
...
冪等性とは、**「同じ操作を何度実行しても、最終的な状態が変わらない」**という性質のこと。（数学的に厳密な定義とは少し異なるが、実務ではこの理解で十分だ。）
...
※ ただし、冪等性の実装には注意が必要だ。「処理済みチェック」と「実際の処理」の間にタイムラグがあると、競合状態（race condition）が発生する可能性がある。本格的な実装では、データベースの一意制約やロックを活用するなど、追加の考慮が必要になることもある。
